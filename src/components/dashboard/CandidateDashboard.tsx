                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <div className="relative group">
                          ...
                          {(job as any).match_analysis && (
                            <div className="absolute bottom-full left-0 mb-2 hidden group-hover:block z-10 w-80 p-3 bg-gray-900 text-white text-xs rounded-lg shadow-lg">
                              <div className="mb-2">
                                <strong>AI Analysis:</strong>
                                <p className="mt-1">{(job as any).match_analysis.reasoning}</p>
                              </div>
                              {(job as any).match_analysis.strengths?.length > 0 && (
                                <div className="mb-2">
                                  <strong className="text-green-300">Strengths:</strong>
                                  <ul className="mt-1 list-disc list-inside">
                                    {(job as any).match_analysis.strengths.slice(0, 2).map((strength: string, idx: number) => (
                                      <li key={idx}>{strength}</li>
                                    ))}
                                  </ul>
                                </div>
                              )}
                              {(job as any).match_analysis.gaps?.length > 0 && (
                                <div>
                                  <strong className="text-yellow-300">Areas to consider:</strong>
                                  <ul className="mt-1 list-disc list-inside">
                                    {(job as any).match_analysis.gaps.slice(0, 2).map((gap: string, idx: number) => (
                                      <li key={idx}>{gap}</li>
                                    ))}
                                  </ul>
                                </div>
